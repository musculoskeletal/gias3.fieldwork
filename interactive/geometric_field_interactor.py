...
class point_picker_3(object):
	...
	def plot_picked_points(self):
		...
		self.picked_plot = self.figure.scene.mlab.points3d( p[:,0], p[:,1], p[:,2] ,mode='sphere', scale_factor=self.pointScaleFactor, color=self.pointColour)
...
class point_picker_3D(object):
	...
	def on_p(self, obj, evt ):
		...
		mlab.points3d( [obj.pick_position[0]], [obj.pick_position[1]], [obj.pick_position[2]], mode='sphere', color=(1,1,0), scale_factor=0.2 )
...
class element_adder_data( object ):
	...
	def __init__( self, geometric_field, data=None, data_mesh=None, data_scalar=None ):
		...
		if data_mesh:
			self.data_plot = mlab.triangular_mesh( data_mesh[0][:,0], data_mesh[0][:,1], data_mesh[0][:,2], data_mesh[1], scalars=data_scalar, figure=self.figure )
			...
			if data != None:
				...
				if data_scalar == None:
					...
					self.data_plot = mlab.points3d( data[0], data[1], data[2], data_scalar, mode='point', figure=self.figure )
		...
		if self.gf.splines:
			for s in self.gf.splines.values():
