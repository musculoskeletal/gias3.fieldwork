...
class mesh_ensemble:
	...
	def save_mesh( self, filename=None, path='', submeshFilenames=None ):
		...
		S['hanging_points'] = [ (h[0], h[1].host_element, h[1].element_coordinates) for h in self.hanging_points.items() ]
	...
	def get_true_elements( self ):
		...
		"""
		elem_list = []
		for element_number in sort( self.elements.keys() ):
			# get element
	...
	def connect_elements_by_edges( self, elem1, edge1, elem2, edge2, xi_extent=None, invert_order=False ):
		...
		if xi_extent:
			...
			if len( elem1_points ) == len( elem2_points ):
				for i in xrange( len( elem1_points ) ):
