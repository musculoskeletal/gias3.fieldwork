...
def makeScalarFieldDerivativesEvaluatorSparse( S, evalD ):
	...
	# matrices
	basisValues = {}
	A = [ scipy.zeros( (ep.shape[0], f.get_number_of_ensemble_points()), dtype=float ) for i in xrange(epd.shape[0]) ]
	row = 0
	for elementNumber in scipy.sort(f.mesh.elements.keys()):
		# get element
		element = f.mesh.elements[ elementNumber ]
...
def makeMaskedDataObj( S, evalD, maskedData, sobW=None, sobD=None, nW=None, nD=None ):
	...
	if (sobW==None) and (nW==None):
		...
		def obj( x ):
			v = evaluator( x )
			d = maskedData - v
			sys.stdout.write( '\rit: %(it)05i rms: %(rms)8.6f'%{'it':c.next(), 'rms':scipy.sqrt((d*d).mean())} )
			sys.stdout.flush()
			return d
		...
		if nW==None:
			...
			def obj( x ):
				...
				sys.stdout.write( '\rit: %(it)05i rms: %(rms)8.6f'%{'it':c.next(), 'rms':scipy.sqrt((dData*dData).mean())} )
				sys.stdout.flush()
				return d
