...
from scipy.linalg import det, norm, svd
...
class geometric_field:
	...
	def write_nodes(self, filename, header=None):
		...
		with open(filename, 'w') as f:
			...
			for n in node_numbers:
				line = '{:6d} '.format(n)+' '.join(['{:12.6f}'.format(x) for x in self.field_parameters[:,n,:].ravel()])+'\n'
				f.write(line)
	...
	def _set_point_ensemble_numbers( self ):
		for pi, ei in self.points_to_ensemble_map.items():
	...
	def remove_geometric_point( self, point_number ):
		...
		for k,v in self.named_points_map.items():
			if v==point_number:
				del self.named_points_map[k]

		# remove from ensemble_to_points_map
		for ep, p  in self.ensemble_to_points_map.items():
			if p == point_number:
	...
	def add_element( self, element, points, debug=0 ):
		...
		if element_points != len( points ):
			raise ValueError, 'ERROR: geometric_field.add_element: number of points '+str(len( points ))+' does not match number of element points '+str(element_points)
	...
	def remove_element( self, element_number ):
		...
		for i, ens in enumerate(en2el_map.keys()):
			elem = en2el_map[ens].keys()[0]
			ep = en2el_map[ens][elem].keys()[0]
		...
		self.points_to_ensemble_map = {p:e for e,p in self.ensemble_to_points_map.items()}
		# update ensemble points attribute of points
		self._set_point_ensemble_numbers()
	...
	def invertSurfaceMesh( self, nodeDistTol=1e-3 ):
		...
		basisTypes = dict([(e,oldEFF.basis[e].type) for e in oldEFF.basis.keys()])
		...
		for elemNumber in M.keys():
			elem = oldEFF.mesh.elements[elemNumber]
			oldNodeNumbers = scipy.array([M[elemNumber][v][0][0] for v in M[elemNumber]])[invertMap[elem.type]]	# look up mapping
	...
	def evaluate_geometric_field( self, density, derivs=None ):
		...
		if not derivs:
			...
			return scipy.array( [self.ensemble_field_function.evaluate_field_in_mesh( density, d, unpack=True ) for d in self.field_parameters] )
	...
	def get_element_numbers( self, coordinates=True ):
		...
		elem_numbers = self.ensemble_field_function.mesh.elements.keys()
		...
		if coordinates:
			...
			for e_i in elem_numbers:
				if self.ensemble_field_function.mesh.elements[e_i].is_element:
					...
					elem_nodes = [v[0][0] for v in self.ensemble_field_function.mapper._element_to_ensemble_map[e_i].values()]
					coords.append(all_node_coords[elem_nodes,:].mean(0))
				else:
					# find centre of mass of region nodes
					region_ens_points = [ v[0][0] for v in self.ensemble_field_function.mapper._element_to_ensemble_map[e_i].values() ]
					region_nodes = scipy.array([ self.get_point_position( self.ensemble_to_points_map[i] ) for i in region_ens_points ])
	...
	def find_closest_material_points( self, dataPoints, initGD=None, verbose=False ):
		...
		elemNumbers = self.ensemble_field_function.mesh.elements.keys()
	...
	def calc_surface_area( self, d, element=None, fullOutput=False ):
		...
		if element!=None:
			...
			ep = scipy.array( [self.ensemble_field_function.evaluate_field_in_mesh(d, p) for p in self.field_parameters] ).T
	...
	#==================================================================#
	def display_geometric_field( self, field_eval_density, point_glyph='sphere', point_label=None, point_scale=1.0, field_glyph='point', field_scale=1.0, data=None, curvature=None, figure=None, scalar=None, name=None ):
	#~ def display_geometric_field( self, field_eval_density, point_glyph='sphere', point_label=None, point_scale=1.0, field_glyph='point', field_scale=1.0, data=None, curvature=None, **mLabArgs ):
		...
		if data != None:
			if data.shape[0]==3:
				...
				mlab.points3d( data[0], data[1], data[2], data[3], mode='point', colormap='jet', vmax=0.2, vmin=-0.2, figure=f )
	...
	def _plot_points( self, glyph = 'sphere', label = None, scale=0.5, figure=None ):
		...
		if len(p)>0:
			...
			if figure:
				points_plot = mlab.points3d( p[:,0], p[:,1], p[:,2], s, mode='sphere', scale_mode='none', scale_factor = scale, color = (1,0,0), figure=figure )
			else:
				points_plot = mlab.points3d( p[:,0], p[:,1], p[:,2], s, mode='sphere', scale_mode='none', scale_factor = scale, color = (1,0,0))
	...
	def _plot_field( self, density, glyph = 'point', scale_factor = 1.0, figure=None ):
		...
		if not self.ensemble_field_function.mesh.get_number_of_elements ():
			...
			if self.dimensions == 3:
				field_plot = mlab.points3d( evaluation[0],evaluation[1],evaluation[2], mode = glyph, scale_factor = 0.8, color = (0.0,0.5,1.0), figure=figure)
	...
	def makeLineElementsFromPoints( self, points, nodesPerElem, elemBasisMap ):
		...
		elemTypes = [elemBasisMap.keys()[0]]*nElems
		x = 0	
		for nE in xrange(nElems):
	...
	def setFitterElementD( self, epD ):
		...
		for ei in scipy.sort(f.mesh.elements.keys()):
	...
	def addElementPointsToFitter( self, fitter ):
		...
		for ei in self.ensemble_field_function.mesh.elements.keys():
	...
	def getElementNodeNumbers( self, element ):
		...
		elemNodeNumbers = [ elemMap[eN][0][0] for eN in elemMap.keys() ]
	...
	def makeGFFromElements( self, name, elements, basisTypes ):
		...
		"""
		newGF = geometric_field( name, self.dimensions, field_dimensions=self.ensemble_field_function.dimensions, field_basis=basisTypes )
		for e in elements:
			...
			elemParams = scipy.array([self.ensemble_field_function.mapper.get_element_parameters(e, p, do_hack=0 ) for p in self.get_field_parameters() ])[:,:,scipy.newaxis]
			try:
	...
	def discretiseAllElementsRegularGeoD( self, maxDistance, geoCoords=False, unpack=True):
		...
		elementOutput = []
		for elementNumber in scipy.sort(self.ensemble_field_function.mesh.elements.keys()):
			if self.ensemble_field_function.mesh.elements[ elementNumber ].is_element==True:
				...
				elementOutput.append( g.discretiseAllElementsRegularGeoD( maxDistance, geoCoords=geoCoords, unpack=unpack ) )
...
def makeGeometricFieldEvaluator( G, evalD ):
	...
	basisValues = {}
	basisMatrices = {}
	for elementNumber in scipy.sort(f.mesh.elements.keys()):
		# get element
...
def makeGeometricFieldEvaluatorSparse( G, evalD, epIndex=None, epXi=None, matPoints=None ):
	...
	if epMode==3:
		...
		for ei, elementNumber in enumerate(scipy.sort(f.mesh.elements.keys())):
			# get element
			element = f.mesh.elements[ elementNumber ]
...
def buildEvaluatorSparse(A, entries, nodeMap, dimensions):
	...
	row = 0
	for eN, b in entries:	
		ensemblePoints = [ nodeMap[eN][n][0][0] for n in xrange(b.shape[0]) ] # element to ensemble map
...
def makeGeometricFieldDerivativesEvaluatorSparse( G, evalD, dim=3, epIndex=None, epXi=None ):
	...
	# matrices
	basisValues = {}
	A = [ scipy.zeros( (nEPs, f.get_number_of_ensemble_points()), dtype=float ) for i in xrange(nDerivs) ]
	row = 0
	for ei, elementNumber in enumerate(scipy.sort(f.mesh.elements.keys())):
		# get element
		element = f.mesh.elements[ elementNumber ]
